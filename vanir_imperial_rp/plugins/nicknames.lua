...
PLUGIN.description = "Allows players to nickname themselves. - Edited by Theodor"
PLUGIN.license = [[
...
ix.command.Add("Nickname", {
    ...
    OnRun = function(self, client, nickname)
        ...
        -- Pattern to find an existing nickname at the end of the name
        local pattern = " '.*'$"
        ...
        if (isstring(nickname) and nickname != "" and nickname != "none") then
            ...
            -- Remove the nickname if it's set to empty or "none"
            name = string.gsub(name, pattern, "")
        end
